...
export default function AppEditorPage({
  ...
  useEffect(() => {
    ...
    loadApp();
  }, [user, authLoading, appId, router, loadApp]);
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        ...
        {previewMode ? (
          /* Preview mode - same as app viewer */
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {activeScreen?.blocks
              ...
              .map((block) => (
                ...
                >
                  <BlockRenderer block={block} appId={appId} />
                </div>
              ))}
          </div>
        ) : (
          ...
          <div className="flex flex-col-reverse gap-6 lg:flex-row">
                            ...
                            <div className="flex gap-1">
                              {WIDTH_OPTIONS.map((w) => (
                                <button
                                  ...
                                  onClick={() =>
                                    updateBlock(block.id, { width: w.value })
                                  }
                                >
                                  {w.label}
                                </button>
            ...
            {/* Add block panel (right side on desktop, top on mobile) */}
            ...
            <div className="flex gap-1">
              {(["large", "compact", "icon"] as const).map((s) => (
                <button
                  ...
                  onClick={() => onConfigChange({ style: s })}
                >
            ...
            <div className="flex gap-1">
              {(["bar", "line", "pie", "area"] as const).map((t) => (
                <button
                  ...
                  className={cn(
                    "rounded border px-2 py-1 text-xs",
                    block.config.chartType === t
                      ? "border-primary bg-primary/10"
                      : "text-muted-foreground"
                  )}
                  onClick={() => onConfigChange({ chartType: t })}
                >
                  {t === "bar"
                    ? "棒"
                    : t === "line"
    ...
    case "metric_card":
      return (
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            <Label className="text-xs">トレンド</Label>
            <div className="flex gap-1">
              {(["up", "down", "flat"] as const).map((t) => (
                <button
                  key={t}
                  className={cn(
                    "rounded border px-2 py-1 text-xs",
                    block.config.trend === t
                      ? "border-primary bg-primary/10"
                      : "text-muted-foreground"
                  )}
                  onClick={() => onConfigChange({ trend: t })}
                >
                  {t === "up" ? "↑" : t === "down" ? "↓" : "→"}
                </button>
              ))}
      ...
      return (
        <div className="space-y-2">
          <div className="space-y-1">
            ...
            <Input
              value={block.config.submitAction || ""}
              onChange={(e) => onConfigChange({ submitAction: e.target.value })}
              placeholder="customers"
              className="h-8 text-sm"
            />
          </div>
          <p className="text-[10px] text-muted-foreground">
            フィールド設定はAIチャットで追加できます
          </p>
        </div>
      );
